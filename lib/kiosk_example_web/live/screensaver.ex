defmodule KioskExampleWeb.Live.Screensaver do
  @moduledoc """
  Provides screensaver functionality for LiveView pages.

  ## Usage

  Add `use KioskExampleWeb.Live.Screensaver` to your LiveView module,
  ...
  defmacro __using__(_opts) do
    quote do
      import KioskExampleWeb.Live.Screensaver,
        only: [init_screensaver: 1, screensaver_events: 0]

      def handle_event("user_activity", _params, socket) do
        {:noreply, KioskExampleWeb.Live.Screensaver.handle_user_activity(socket)}
      end

      def handle_info(:check_idle, socket) do
        {:noreply, KioskExampleWeb.Live.Screensaver.handle_idle_check(socket)}
      end
    end
  end
